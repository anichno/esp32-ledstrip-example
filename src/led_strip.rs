...
impl Color {
    ...
    pub fn to_array(self, brightness: f32) -> [u8; 3] {
        [
            clamp(
                (self.g as f32 / 256f32 * MAX_BRIGHTNESS as f32 * brightness) as u8,
                0,
                MAX_BRIGHTNESS,
            ),
            clamp(
                (self.r as f32 / 256f32 * MAX_BRIGHTNESS as f32 * brightness) as u8,
                0,
                MAX_BRIGHTNESS,
            ),
            clamp(
                (self.b as f32 / 256f32 * MAX_BRIGHTNESS as f32 * brightness) as u8,
                0,
                MAX_BRIGHTNESS,
            ),
...
impl<const NUM_LEDS: usize> LedStrip<NUM_LEDS> {
    pub fn new(channel: rmt_channel_t, gpio_num: gpio_num_t) -> Result<Self, EspError> {
        ...
        println!(
            "zero_high_ticks {}, zero_low_ticks {}, one_high_ticks {}, one_low_ticks {}, ",
            zero_high_ticks, zero_low_ticks, one_high_ticks, one_low_ticks
        );
